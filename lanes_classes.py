...
class lanesConfig(Config):
    ...
    NUM_CLASSES = 1 + 5  # Background + no_class
    ...
    IMAGE_SHAPE = np.array((400, 640, 3))
...
class lanesDataset(utils.Dataset):
    ...
    def load_lanes(self, subset):
        ...
        fimgs = glob.glob('/media/ml3/Volume/sg/rei_class_instances_sentosa/images/*.png')
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        init_mask = cv2.imread('/media/ml3/Volume/sg/rei_class_instances_sentosa/instances/'+img_name)[:, :, 0]
        

        val = []
        no_class = -5
        for k in range(5):
            no_class += np.unique(init_mask[k*400:(k+1)*400]).shape[0]

            for i in range(1, np.unique(init_mask[k*400:(k+1)*400]).shape[0]):
                val.append(np.unique(init_mask[k*400:(k+1)*400])[i])

        
        #instances
        mask = np.zeros((400, 640, no_class))
        label = []
        
        for k in range(5):

            class_mask = init_mask[k*400: (k+1)*400, :]
            
            for i in range(0,no_class):
                
                ind = class_mask == val[i]
                # print(ind.shape)
                if np.sum(ind) == 0:
                    continue
...
def color_splash(image, mask, class_ids):
    ...
    color = [[255, 255, 0], [255, 0, 255], [255, 0, 0], [0, 255, 255], [0, 255, 0], [0, 0, 255]]
    ...
    if mask.shape[1] == image.shape[1] and mask.shape[0] == image.shape[0]:
        ...
        for i in range(mask.shape[2]):
            ...
            lanes[mask[:, :, i] > 0] = color[class_ids[i]%len(color)]
